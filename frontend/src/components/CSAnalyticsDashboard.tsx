...
const CSAnalyticsDashboard = ({
  ...
  const [selectedMetric, setSelectedMetric] = useState<"rating" | "mood">(
    "rating",
  );
  const [sortBy, setSortBy] = useState<"name" | "rating" | "interactions">(
    "rating",
  );
  ...
  const teamAverageRating = sortedData.length
    ? sortedData.reduce((sum, agent) => sum + agent.averageRating, 0) /
      sortedData.length
    : 0;

  const totalMoodChanges = sortedData.reduce(
    (total, agent) => {
      return {
  ...
  const totalInteractions = sortedData.reduce(
    (sum, agent) => sum + agent.totalInteractions,
    0,
  );
  const positiveRatio = totalInteractions
    ? totalMoodChanges.positive / totalInteractions
    : 0;
  const negativeRatio = totalInteractions
    ? totalMoodChanges.negative / totalInteractions
    : 0;
  ...
  const displayRatingData = sortedData.length
  ...
  const displayMoodData = sortedData.length
    ? moodChangeData
    : sampleData.map((agent) => ({
        name: agent.agentName,
        positive: agent.moodChanges.positive,
          ...
          <div className="bg-gray-50 p-4 rounded-lg">
            ...
            <div className="flex items-center">
              <p className="text-2xl font-bold">
                {sortedData.length ? teamAverageRating.toFixed(2) : "4.2"}
              </p>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {displayData.map((agent) => {
                  const positivePercentage = agent.totalInteractions
                    ? (
                        (agent.moodChanges.positive / agent.totalInteractions) *
                        100
                      ).toFixed(1)
                    : "0.0";
                  const negativePercentage = agent.totalInteractions
                    ? (
                        (agent.moodChanges.negative / agent.totalInteractions) *
                        100
                      ).toFixed(1)
                    : "0.0";
